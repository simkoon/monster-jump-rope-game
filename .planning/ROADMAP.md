...
Plans:
- [ ] 01-01: 프로젝트 스캐폴드(Vite/React 19/TS/R3F/Zustand persist/Zod), 버전 포함 schema.ts, 콘텐츠 레포, read-guard 포함 localStorage 자동 저장
- [ ] 01-02: 미션 편집기(CRUD·검색·난이도·카테고리) + 이벤트 편집기(CRUD·효과 종류·확률/가중치)
- [ ] 01-03: schemaVersion + Zod 검증 포함 JSON 내보내기/가져오기 (잘못된 파일은 덮어쓰지 않고 오류 안내)
