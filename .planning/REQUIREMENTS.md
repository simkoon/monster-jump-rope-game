...
- [x] **LOOP-06**: 주사위 값만큼 보드판 위에서 현재 플레이어의 말(토큰)이 칸을 이동하는 애니메이션이 재생된다 (Phase 3에서 3D로 충족 → 2026-07-26 2D 전환 결정에 따라 Phase 3.1에서 2D로 재구현)
...
- [ ] **LOOP-11**: 보드는 진행 방향과 결승 칸의 위치를 시각적으로 명확히 드러내며, 칸은 서로 구분되어 개별로 셀 수 있다 (2026-07-26 사용자 피드백)
- [ ] **LOOP-12**: 각 플레이어/팀의 진행 정도와 결승까지 남은 거리가 서로 비교 가능하게 표시되어, 누가 앞서는지 즉시 알 수 있다 (2026-07-26 사용자 피드백)
...
- [ ] **ART-02**: 남자 캐릭터와 여자 캐릭터가 각각 2D 스프라이트로 제공되며 줄넘기를 든 모습을 포함한다 (2026-07-26 2D 전환 결정)
- [ ] **ART-03**: 캐릭터는 상황(성공/실패/이동 등)에 따라 다양한 표정을 보여준다
- [x] **ART-04**: 버튼은 크고 단순하며 텍스트를 최소화해 아동이 쉽게 조작할 수 있다
...
| LOOP-06 | Phase 3 → Phase 3.1 | Re-implementing (3D→2D) |
| LOOP-11 | Phase 3.1 | Pending |
| LOOP-12 | Phase 3.1 | Pending |
...
| ART-04 | Phase 3 (Phase 3.1에서 유지 검증) | Complete |
